...
class CalcDIC: 
    ...
    def calculate_dic_driftcorr(self, tobe_driftcorr_list, time_frame): 
        ...
        if column_names.dic_crm_counts_per_umol_driftcorr in self.df.columns:
            pass
        else:
            ...
            try: 
                if time_frame == "single_days":
                    for corr_day in tobe_driftcorr_list:
                        ...
                        self.df[column_names.dic_driftcorr_flag][self.df[column_names.analysisdate] == corr_day] = 1

                        # Extract the relevant variables and convert them to an array and reshape so it can be used by the LinearRegression class
                        crm_time_array = self.df[column_names.analysistime][(self.df[column_names.analysisdate] == corr_day) & (self.df[column_names.station] == column_names.crmid)].values.reshape(-1,1)
                        samples_time_array = self.df[column_names.analysistime][self.df[column_names.analysisdate] == corr_day].values.reshape(-1,1)
                        crm_ctsperumol_array = self.df[column_names.dic_crm_counts_per_umol][(self.df[column_names.analysisdate] == corr_day) & (self.df[column_names.station] == column_names.crmid)].values.reshape(-1,1)
                        ...
                        for i in samples_time_array:
                            ...
                            ix = self.df[(self.df[column_names.analysisdate] == corr_day) & (self.df[column_names.analysistime] == i[0])].index
                    ...
                    self.df[column_names.dic_driftcorr_flag][(self.df[column_names.analysisdate]>date_min) & (self.df[column_names.analysisdate] < date_max)] = 1 
                
                    # Extract the relevant variables and convert to array + reshape for use in LinearRegression
                    crm_time_array = self.df[column_names.analysistime][(self.df[column_names.analysisdate] > date_min) & (self.df[column_names.analysisdate] < date_max) & (self.df[column_names.station] == column_names.crmid)].values.reshape(-1,1)
                    samples_time_array = self.df[column_names.analysistime][(self.df[column_names.analysisdate] > date_min) & (self.df[column_names.analysisdate] < date_max) & (self.df[column_names.station] != column_names.crmid)].values.reshape(-1,1)
                    crm_ctsperumol_array = self.df[column_names.dic_crm_counts_per_umol][(self.df[column_names.analysisdate] > date_min) & (self.df[column_names.analysisdate] < date_max) & (self.df[column_names.station] == column_names.crmid)].values.reshape(-1,1)
                    ...
                    # Per sample, determine the counts/umol based on the linear regression
                    for i in self.df[(self.df[column_names.analysisdate]>date_min) & (self.df[column_names.analysisdate] < date_max) & (self.df[column_names.station] != column_names.crmid)].index: 
                    
                        crm_drift_corr = linear_regressor.predict(self.df[column_names.analysistime][i].reshape(-1,1))[0][0]
                    
                        # Save drift corrected value 
                        self.df.loc[i, column_names.crm_dic_counts_per_umol_driftcorr] = crm_drift_corr
